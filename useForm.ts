...
function setNestedObjectValues(object: any, value: any, visited = new WeakMap(), response = {} as any) {
...
export function useForm<T extends UseFormKeyVal>(props: UseFormProps<T>) {
  ...
  useDisposable(() => autorun(validate));
  ...
  const getFieldError = (fieldName: string) => (state.touched[fieldName] ? state.errors[fieldName] : null);
